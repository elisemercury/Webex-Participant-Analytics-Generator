    ...
    try:
        ...
        for participant in particpant_info:
            ...
            if participant["email"][0:7] == "machine" and participant["devices"][0]["deviceType"] == "tp_endpoint":
                ...
                worksheet.write_datetime(row, col+3, joined.time(), date_format)
                worksheet.write_datetime(row, col+4, left.time(), date_format)
                worksheet.write(row, col+5, str(timezone.tzname()) + " " + str(timezone)[19:])

                total = left - joined 

                worksheet.write_datetime(row, col+6, total, date_format)
                row += 1
